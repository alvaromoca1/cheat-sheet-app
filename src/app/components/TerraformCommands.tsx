...
import data from '../terraform.json'
...
export default function TerraformCommands() {
  const [dockerCommands, setTerraformCommands] = useState<TerraformCommand[]>([])
  const [searchTerm, setSearchTerm] = useState('')
  const [filteredCommands, setFilteredCommands] = useState<TerraformCommand[]>([])
  const [expandedCards, setExpandedCards] = useState<{ [key: string]: boolean }>({})

  useEffect(() => {
    setTerraformCommands(data)
  ...
  return (
    <div className={styles.container}>
      <h1 className={styles.title}>Comandos de Kubernetes</h1>
